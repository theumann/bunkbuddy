...
describe.sequential("Matches", () => {
  ...
  describe("Matches Filtering and Sorting", () => {
    it("filters candidates by 3-digit targetZip prefix and excludes self", async () => {
      const me = await signupUser(ctx, {
        targetZip: "94117",
        displayName: "me1",
      });

      const samePrefix1 = await signupUser(ctx, {
        targetZip: "94110",
        displayName: "same1",
      });
      const samePrefix2 = await signupUser(ctx, {
        targetZip: "94199",
        displayName: "same2",
      });

      const otherPrefix = await signupUser(ctx, {
        targetZip: "94016",
        displayName: "other",
      }); // different prefix
      ...
      expect(items.some((x: any) => x.userId === samePrefix1.userId)).toBe(
        true,
      );
      expect(items.some((x: any) => x.userId === samePrefix2.userId)).toBe(
        true,
      );

      // exclude different prefix users
      expect(items.some((x: any) => x.userId === otherPrefix.userId)).toBe(
        false,
      );
    });
  ...
  describe("GET /matches ordering", () => {
    it("orders scored first by score desc, then createdAt desc", async () => {
      ...
      const C = await signupUser(ctx, {
        displayName: "CCC",
        targetZip: "94112",
      }); // same prefix (but unscored)

      // set createdAt just so ties are deterministic if needed
      await setProfileCreatedAt(
        ctx,
        A.userId,
      ...
      await answerQuestions(ctx, C.token, [
        { questionId: qs[0].id, value: "A" },
      ]); // 1/10 = 10%
    ...
    it("orders unscored by same zip first, then createdAt desc", async () => {
      ...
      const sameZipOld = await signupUser(ctx, {
        displayName: "sameZipOld",
        targetZip: "94117",
      });
      const sameZipNew = await signupUser(ctx, {
        displayName: "sameZipNew",
        targetZip: "94117",
      });
      const nearbyDiffZip = await signupUser(ctx, {
        displayName: "nearbyDiffZip",
        targetZip: "94110",
      });

      await answerQuestions(ctx, sameZipOld.token, [
        { questionId: qs[0].id, value: "A" },
      ]);
      await answerQuestions(ctx, sameZipNew.token, [
        { questionId: qs[0].id, value: "A" },
      ]);
      await answerQuestions(ctx, nearbyDiffZip.token, [
        { questionId: qs[0].id, value: "A" },
      ]);
      ...
      // Find positions
      const idxOld = items.findIndex(
        (x: any) => x.userId === sameZipOld.userId,
      );
      const idxNew = items.findIndex(
        (x: any) => x.userId === sameZipNew.userId,
      );
      const idxNear = items.findIndex(
        (x: any) => x.userId === nearbyDiffZip.userId,
      );
