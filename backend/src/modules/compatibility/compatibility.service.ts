...
export async function getUserCompatibilityStats(prisma: PrismaClient, userId: string) {
  const activeQuestions = await prisma.compatibilityQuestion.findMany({
    ...
    const activeQuestionIds = activeQuestions.map(
    (q: { id: string }) => q.id
    );
 ...
 */
  ...
  export async function updateMyAnswers(
    ...
    for (const input of toUpsert) {
      ...
      if (!q) {
        const err: any = new Error("One or more questions are invalid or inactive");
        err.status = 400;
        throw err;
      }
      ...
      if (q.options && Array.isArray(q.options)) {
        ...
        if (!options.includes(input.value)) {
          const err: any = new Error(`Invalid value for question ${q.code || q.id}`);
          err.status = 400;
          throw err;
        }
      }
